...
function safeToDate(input, fallback = null) {
  try {
    ...
    if (typeof input === "object") {
      ...
      if (input.$date != null) return safeToDate(input.$date, fallback);

      // nested date shapes
      if (input.date != null) return safeToDate(input.date, fallback);
      if (input.value != null) return safeToDate(input.value, fallback);
    }
...
/** Format using locale date safely (do NOT fallback to today) */
...
export default function Student({ onOpenAuth, setRoute }) {
  ...
  async function handleDownloadCertificate() {
    try {
      ...
      const logoSize = 64;
      try {
        const logo = await loadImage("/ictbanner.jpg");
      ...
      doc.setFontSize(15);
      doc.text("ICT Mastermind", headerLeft, 65);
      ...
      doc.setTextColor(...GREY);
      doc.setFontSize(9);
      // --- Footer Disclaimer (wrapped, bottom-safe) ---
      doc.setFont("helvetica", "normal");
      doc.setTextColor(...GREY);
      doc.setFontSize(9);

      const footerMarginX = 52;
      const footerMaxW = W - footerMarginX * 2;
      const footerBottomPad = 40; // distance from page bottom
      ...
      const disclaimerLines = doc.splitTextToSize(disclaimer, footerMaxW);
      const lineH = 11; // good for 9pt font
      const startY = H - footerBottomPad - (disclaimerLines.length - 1) * lineH;

      doc.text(disclaimerLines, W / 2, startY, { align: "center" });
